...
export default function OutputLanguageSelect({
  ...
  // ✅ 핵심: select에 "현재 선택값"이 quick에 없으면 임시 옵션으로 추가
  const selectOptions = useMemo(() => {
    const inQuick = quick.some((o) => o.code === value);
    if (inQuick) return quick;

    // value가 quick에 없는데 all에는 있는 경우: "현재 선택" 옵션으로 보여주기
        ...
        >
          ...
          >
            ...
            <div className="mt-4">
              ...
              <div className="mt-3 max-h-[360px] overflow-auto rounded-2xl border border-neutral-200 bg-neutral-50 p-2 dark:border-white/12 dark:bg-white/5">
                    ...
                    return (
                      <button
                        ...
                        className={`
                          ...
                          ${
                            active
                              ? "border-blue-300 bg-white shadow-sm dark:border-[rgb(var(--hero-b))]/60 dark:bg-black/35"
                              : "border-neutral-200 bg-white hover:bg-neutral-100 dark:border-white/12 dark:bg-black/25 dark:hover:bg-black/35"
                          }
                        `}
                      >
                ...
                {filtered.length === 0 && (
                  <div className="px-3 py-8 text-center text-sm text-neutral-600 dark:text-neutral-300">
              ...
              <div className="mt-3 text-xs text-neutral-500 dark:text-neutral-400">
                현재 선택:{" "}
                <b className="text-neutral-900 dark:text-neutral-200">{currentLabel}</b>{" "}
                <span className="mx-1 text-neutral-300 dark:text-white/20">·</span>
                코드: <b className="text-neutral-900 dark:text-neutral-200">{value}</b>
              </div>
            </div>
          </div>
        </div>
