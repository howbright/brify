...
export default function SignupCompletePage() {
  ...
  useEffect(() => {
    ...
    async function boot() {
      ...
      // ✅ OAuth 직후 세션이 늦게 붙는 경우가 있어서 재시도
      ...
      for (let i = 0; i < 10; i++) {
        ...
        await sleep(150); // 150ms * 10 = 최대 1.5초 기다림
        if (!alive) return;
      }

      if (!alive) return;

      // ✅ 세션이 없더라도 complete 화면은 보여주되,
      //    uid/sig가 없다면 API로도 처리 불가하니 안내
      if (!sessionUser) {
        setLoading(false);

        if (!hasSignedUid) {
          setMessage("가입 완료 링크 정보(uid/sig)가 없어 처리를 진행할 수 없습니다. 다시 로그인/가입을 시도해 주세요.");
          setMessageType("error");
        } else {
          setMessage("로그인 정보를 확인하는 중입니다. 잠시 후 동의 후 진행해 주세요.");
          setMessageType("error");
        }
        return;
      }

      // profiles 존재/terms_accepted 확인
  ...
  }, [router, next, supabase, hasSignedUid]);
  ...
  async function handleComplete() {
    ...
    // ✅ uid/sig 없으면 서버에서 검증 불가(세션 없어도 처리하려는 구조라면 특히)
    if (!hasSignedUid) {
      setMessage("가입 완료 링크 정보(uid/sig)가 없어 처리를 진행할 수 없습니다. 다시 로그인/가입을 시도해 주세요.");
      setMessageType("error");
      return;
    ...
    try {
      ...
      if (!r.ok || !j?.ok) {
        setSubmitting(false);
        setMessage(`가입 완료 처리 중 오류가 발생했습니다. (${j?.error ?? "UNKNOWN"})`);
        setMessageType("error");
        return;
      }
      ...
      <div className="rounded-3xl border border-neutral-200 bg-white/95 shadow-[0_22px_45px_-28px_rgba(15,23,42,0.35)] dark:border-white/12 dark:bg-[#020617]">
        <div className="flex flex-col gap-6 p-6 sm:p-8">
          ...
          <p className="text-center text-xs text-neutral-500 dark:text-neutral-400">
            동의 후 서비스 이용이 가능합니다. 동의 내용을 확인하려면 약관 링크를 눌러 주세요.
          </p>
        </div>
      </div>
    </div>
