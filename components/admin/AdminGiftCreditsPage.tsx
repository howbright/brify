...
export default function AdminGiftCreditsPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (!selectedUser?.id) {
        throw new Error("지급할 사용자를 먼저 검색해서 선택해 주세요.");
      }
        ...
        <section className="mt-8 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          >
            ...
            <p className="mt-1 text-sm leading-6 text-neutral-600">
              이메일로 사용자를 검색한 뒤, 선택된 사용자에게 free credits를 지급합니다.
            </p>

            <div className="mt-5 space-y-4">
              <label className="block">
                <div className="text-sm font-semibold text-neutral-800">사용자 검색</div>
                <input
                  type="text"
                  required
                  value={searchText}
                  ...
                  placeholder="이메일 일부를 입력해 사용자를 검색해 주세요."
                ...
                <div className="mt-2 min-h-6 text-xs text-neutral-500">
                  {searching
                    ...
                    : searchText.trim().length < 2
                      ? "이메일 2글자 이상 입력하면 후보가 나타나요."
                      : searchResults.length > 0
                        ? "아래 후보 중 지급 대상을 선택해 주세요."
                        : "일치하는 사용자가 없어요."}
                </div>
                {searchResults.length > 0 ? (
                    ...
                    {searchResults.map((user) => {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "flex w-full items-start justify-between gap-4 border-b border-slate-200 px-4 py-3 text-left last:border-b-0",
                        ...
                        >
                          <div className="min-w-0">
                            ...
                            >
                              {user.role} · free {user.creditsFree.toLocaleString()} · total{" "}
                          ...
                          <div
                            className={cn(
                              "shrink-0 text-[11px]",
                              isSelected ? "text-slate-300" : "text-neutral-400"
                            )}
                          >
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3">
                <div className="text-sm font-semibold text-neutral-800">선택된 사용자</div>
                {selectedUser ? (
                  <div className="mt-2 space-y-1 text-sm text-neutral-700">
                    <div className="font-semibold text-neutral-950">
                      {selectedUser.email ?? "(email 없음)"}
                    </div>
                    <div>userId: {selectedUser.id}</div>
                    <div>
                      role: {selectedUser.role} · free {selectedUser.creditsFree.toLocaleString()} · paid{" "}
                      {selectedUser.creditsPaid.toLocaleString()} · total{" "}
