...
export default function MapPreviewPanel({
  ...
  if (!draft) {
    return (
      <aside className="rounded-2xl border border-slate-400 bg-white p-5 text-sm text-neutral-500 dark:border-white/20 dark:bg-white/[0.04] dark:text-white/60">
        {emptyMessage}
      </aside>
    );
  }
      ...
      <aside className="rounded-2xl border border-slate-400 bg-white p-5 dark:border-white/20 dark:bg-white/[0.04]">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            type="button"
            onClick={onOpen}
            className="inline-flex items-center justify-center rounded-full border border-slate-400 bg-white px-3 py-1.5 text-xs font-semibold text-neutral-700 hover:bg-neutral-50 dark:border-white/20 dark:bg-white/[0.06] dark:text-white/85 dark:hover:bg-white/10"
          >
            미리보기 열기
          </button>
        </div>
    ...
    <aside className="rounded-2xl border border-slate-400 bg-white p-5 dark:border-white/20 dark:bg-white/[0.04]">
      ...
      {draft.tags?.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-1.5">
          {draft.tags.slice(0, 10).map((tag) => (
            <span
              key={tag}
              className="rounded-full border border-slate-400 bg-neutral-50 px-2 py-0.5 text-[11px] text-neutral-600 dark:border-white/20 dark:bg-white/[0.06] dark:text-white/75"
            >
              #{tag}
            </span>
          ))}
      ...
      <div className="mt-4 relative">
        {previewStatus === "loading" && (
          <div className="flex min-h-[180px] items-center justify-center rounded-2xl border border-slate-400 bg-neutral-50 text-xs text-neutral-500 dark:border-white/20 dark:bg-white/[0.04] dark:text-white/60">
            미리보기 불러오는 중…
          </div>
        )}
        ...
        {previewStatus === "loaded" && (
          <div className="absolute bottom-3 right-3 flex items-center rounded-full border border-slate-400 bg-white/90 text-xs font-semibold text-neutral-600 shadow-sm backdrop-blur dark:border-white/20 dark:bg-[#0b1220]/75 dark:text-white/80">
            <button
              type="button"
